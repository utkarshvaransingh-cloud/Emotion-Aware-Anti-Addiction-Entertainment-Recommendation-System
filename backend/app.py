...
try:
    print("Initializing FinalRanker...")
    ranker = FinalRanker()
    print("FinalRanker initialized successfully.")
except Exception as e:
    print(f"CRITICAL ERROR: Failed to initialize FinalRanker: {e}")
    import traceback
    traceback.print_exc()
    ranker = None
...
def health_check():
    if ranker is None:
        return {"status": "error", "message": "Ranker not initialized"}
...
def get_recommendations(request: RecommendRequest):
    ...
    """
    if ranker is None:
        raise HTTPException(status_code=503, detail="Ranker service not available")
    try:
        # 1. Build State
        ...
        return {
            ...
            "meta": {
                ...
                "fatigue_score": state["fatigue"].get("score")
            }
        }
    except Exception as e:
...
if __name__ == "__main__":
    import uvicorn
    print("Starting Uvicorn server on 0.0.0.0:8080...")
